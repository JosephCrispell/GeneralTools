...
badgerInfo <- read.table(
  file="C:/Users/Joseph Crisp/Dropbox/SummerStudents/BadgerInformation.csv",
  header=TRUE,
  sep=",")
```
...
We used the function `head()`. `head()` is a function that will by default print the first 6 rows of a `data.frame`. `data.frame`s in R provide a means of storing a table, with rows and columns. `badgerInfo` is a table with:

```{r}
dim(badgerInfo)
```

The `dim()` function will tell us how many rows (100) and columns (3) are in the `data.frame` (`badgerInfo`) we provide it with.
...
We used the `head()` function here so that we didn't end up printing too many values to the screen. When you want all the values
...
So now we have a table with some data, and we know how to access the data in the table, lets get plotting. First lets see what the weights of the badgers look like:
```{r}
hist(badgerInfo[, "Weight"])
```

The `hist()` function will plot any set of numbers as a histogram - here we gave the `hist()` function the badger weights (we sneakily used the columns name "Weight" as its index).
...
How does a badgers weight relate to its length?
We can make the plot look a lot better by adding some titles:
```{r}
...
Lastly, how does the badger's sex relate to their weights?
```{r}
...
The `boxplot()` takes its data in the form of a formula: `response ~ explanatory`. Where the explanatory data is the data that we are trying to used to explain the variation we observed in the response data.
...
Plot the badgers lengths against their lengths using the same code as above but colour the points red and make points be shaped like triangles.
...
Plotting the provides an easy and quick way to see what's going on with our data. To learn a bit more, there are loads of tutorials available online showing you how to make some amazing plots. Here are a few to get started with:
