...
sub checkForAlleleToRescue{
	...
	if(exists($alleles{$allelesPresent[$chosen]}) && $alleles{$allelesPresent[$chosen]} >= $nIsolatesAlleleMustBePresentIn &&
	$depthForAllelesPresent[$chosen] >= $depthThreshold && $proportionHighQualityBasesSupportingAllele >= $supportThreshold){
