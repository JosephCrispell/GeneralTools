...
# Open a PDF
file <- paste(path, "BASTA/DefinedDemes_6Demes_", date, ".pdf", sep="")
pdf(file)
...
# Add lines to split the demes in two
points(x=c(badgerCentre[1], badgerCentre[1]), 
       y=c(badgerCentre[2] - outerDistance,
           badgerCentre[2] - thresholdDistance),
       type="l")

points(x=c(badgerCentre[1], badgerCentre[1]), 
       y=c(badgerCentre[2] + thresholdDistance,
           badgerCentre[2] + outerDistance),
       type="l")

# Calculate the distance of all isolates from centre of badger group
...
plot(y=selectedIsolatesInfo$Distance, x=selectedIsolatesInfo$X,
     col=ifelse(selectedIsolatesInfo$Species == "COW", rgb(0,0,1, 0.75),
                rgb(1,0,0, 0.75)),
     xaxt="n", xlab="", ylab="Distance (m)", main="Distance to Badger Centre Point", las=1,
     pch=ifelse(selectedIsolatesInfo$Species == "COW", 17, 16))
legend("top", legend=c("Cow", "Badger"),
       text.col=c("blue", "red"), cex=1, bty='n')
points(x=c(min(selectedIsolatesInfo$X, na.rm=TRUE), max(selectedIsolatesInfo$X, na.rm=TRUE)),
       y=c(thresholdDistance, thresholdDistance), type="l", lty=2, col="black")

dev.off()

# Define demes
# A - Inner
# B - Outer East of centre
# C - Outer West of centre
# Each deme is then split by species

selectedIsolatesInfo$Deme <- rep("NA", nrow(selectedIsolatesInfo))
for(row in 1:nrow(selectedIsolatesInfo)){
  
  species <- "Badger"
  if(selectedIsolatesInfo[row, "Species"] == "COW"){
    species <- "Cow"
  }
  ...
  if(is.na(selectedIsolatesInfo[row, "X"]) == FALSE){
    ...
    if(selectedIsolatesInfo[row, "X"] <= badgerCentre[1]){
      side <- "West"
    }
  }
  
  if(distance == "Inner"){
    selectedIsolatesInfo[row, "Deme"] <- paste(species, distance, sep="")
  }else{
    selectedIsolatesInfo[row, "Deme"] <- paste(species, distance, side, sep="")
  }
}

# Check deme assignment
plot(y=selectedIsolatesInfo$Y, x=selectedIsolatesInfo$X,
     col=ifelse(selectedIsolatesInfo$Species == "COW", rgb(0,0,1, 0.75),
                rgb(1,0,0, 0.75)),
     xaxt="n", xlab="", ylab="Distance (m)", main="Distance to Badger Centre Point", las=1, pch=19)
text(y=selectedIsolatesInfo$Y, x=selectedIsolatesInfo$X, 
     labels=selectedIsolatesInfo$Deme, cex=0.5)
...
# Open an output file
fileName <- paste(path, "BASTA/", "xmlInput_6Demes_", date, ".xml", sep="")
