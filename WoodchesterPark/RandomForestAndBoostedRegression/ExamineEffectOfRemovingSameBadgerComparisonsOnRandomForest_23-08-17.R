...
# Select only comparisons between animals
geneticVsEpi <- geneticVsEpi[geneticVsEpi$SameAnimal == 0, ]
...
######################################################
# Check no comparisons between the same animal exist #
######################################################
...
# Check each comparison in geneticVsEpi table
checkThatNoSameAnimalComparisonsRemain(geneticVsEpi, isolateTattoos)
...
checkThatNoSameAnimalComparisonsRemain <- function(geneticVsEpi,
                                                   isolateTattoos){
  for(row in 1:nrow(geneticVsEpi)){
    
    if(isolateTattoos[[geneticVsEpi[row, "IsolateI"]]] == 
       isolateTattoos[[geneticVsEpi[row, "IsolateJ"]]]){
      
      cat("Houston we have a massive problem!\n")
    }
  }
}
