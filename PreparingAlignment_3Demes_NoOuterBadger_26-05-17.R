...
# Open a PDF
file <- paste(path, "BASTA/DefinedDemes_3Demes_NoOuterBadger_", date, ".pdf", sep="")
pdf(file)
...
for(row in 1:nrow(selectedIsolatesInfo)){
  ...
  if(is.na(selectedIsolatesInfo[row, "X"]) == FALSE){
    ...
    if(selectedIsolatesInfo[row, "Distance"] <= thresholdDistance){
      ...
      selectedIsolatesInfo[row, "Deme"] <- "outer"
    }
...
# Open an output file
fileName <- paste(path, "BASTA/", "xmlInput_3Demes_NoOuterBadgers_", date, ".xml", sep="")
