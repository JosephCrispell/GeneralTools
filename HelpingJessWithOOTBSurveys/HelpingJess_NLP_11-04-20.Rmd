...
library(textdata) # Download, parse, store, and load text datasets
...
calculateSentimentScore <- function(sentences){
  
  # Get the AFINN-11 sentiment lexicon
  sentimentLexicon <- as.data.frame(lexicon_afinn())
...
```{r sentiment analysis of whether retail wanted, echo=FALSE}
# Calculate the sentiment score for each answer to the retail question
sentimentScores <- calculateSentimentScore(answers$retail$answer)

# Plot the sentiment scores
hist(sentimentScores, xlim=c(-5, 5), las=1,
     xlab="Sentiment score",
     main="Sentiment of responses about a retail space at AMS")
```

```{r sentiment analyses, echo=FALSE}
# Calculate the sentiment score for each answer to the exhibition space question
sentimentScoresExhibition <- calculateSentimentScore(answers$exhibition$answer)

# Calculate the sentiment score for each answer to the retail space question
sentimentScoresRetail <- calculateSentimentScore(answers$retail$answer)
